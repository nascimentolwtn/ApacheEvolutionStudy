...
public class CommitLineTest {
	...
	private String wrongHeader = CommitLine.WRONG_OUTPUT_HEADER;
	private String invisibleCharHeader = CommitLine.INVISIBLE_CHAR0_OUTPUT_HEADER;
	...
	public void invalidParseWrongHeader() {
		try {
			CommitLine.parseCommitLine(wrongHeader, CommitLineType.INPUT);
			fail("Shoud throw Unparse Exception");
		} catch (RuntimeException e) {
			Assert.assertSame(NonParseableCommitLineException.class, e.getClass());
			assertEquals("Should not parse the CSV header.", e.getMessage());
	...
	public void invalidParseInvisibleCharHeader() {
		try {
			CommitLine.parseCommitLine(invisibleCharHeader, CommitLineType.OUTPUT);
			fail("Shoud throw Unparse Exception");
		} catch (RuntimeException e) {
			Assert.assertSame(NonParseableCommitLineException.class, e.getClass());
			assertEquals("Should not parse the CSV header.", e.getMessage());
