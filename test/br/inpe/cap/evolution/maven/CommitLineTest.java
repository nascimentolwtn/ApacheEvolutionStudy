...
import static br.inpe.cap.evolution.maven.CommitLine.INVISIBLE_CHAR0_OUTPUT_HEADER;
import static br.inpe.cap.evolution.maven.CommitLine.OUTPUT_HEADER;
import static br.inpe.cap.evolution.maven.CommitLine.WRONG_OUTPUT_HEADER;
...
public class CommitLineTest {
	...
	public void invalidParseHeader() {
		try {
			CommitLine.parseCommitLine(OUTPUT_HEADER, CommitLineType.INPUT);
			fail("Shoud throw Unparse Exception");
		} catch (RuntimeException e) {
			Assert.assertSame(NonParseableCommitLineException.class, e.getClass());
	...
	@Test
	public void invalidParseQuotedHeader() {
		try {
			CommitLine.parseCommitLine("\""+OUTPUT_HEADER+"\"", CommitLineType.INPUT);
			fail("Shoud throw Unparse Exception");
		} catch (RuntimeException e) {
	...
	@Test
	public void invalidParseWrongHeader() {
		try {
			CommitLine.parseCommitLine(WRONG_OUTPUT_HEADER, CommitLineType.INPUT);
			fail("Shoud throw Unparse Exception");
		} catch (RuntimeException e) {
			Assert.assertSame(NonParseableCommitLineException.class, e.getClass());
			assertEquals("Should not parse the CSV header.", e.getMessage());
	...
	@Test
	public void invalidParseInvisibleCharHeader() {
		try {
			CommitLine.parseCommitLine(INVISIBLE_CHAR0_OUTPUT_HEADER, CommitLineType.OUTPUT);
			fail("Shoud throw Unparse Exception");
		} catch (RuntimeException e) {
			Assert.assertSame(NonParseableCommitLineException.class, e.getClass());
			assertEquals("Should not parse the CSV header.", e.getMessage());
